Function Get-HPProductNumberAndSerialNumber {
    ...
    try {
        ...
        if ($manufacturer -eq 'Hewlett-Packard' -or $manufacturer -eq 'HP') {
            return [PSObject] @{
                SerialNumber = (Get-WmiObject -Namespace 'root\cimV2' -Class 'Win32_Bios' -ComputerName $ComputerName -ErrorAction Stop).SerialNumber.Trim()
                ProductNumber = (Get-WmiObject -Namespace 'root\WMI' -Class 'MS_SystemInformation' -ComputerName $ComputerName -ErrorAction Stop).SystemSKU.Trim()
            }
        } else {
